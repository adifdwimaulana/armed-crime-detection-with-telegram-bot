...
# Global Vars
firebase = firebase.FirebaseApplication('https://crime-detection-hology.firebaseio.com/', None)
t = time.localtime()
...
def sendFirebase():
    firebase.put('/', "status", 1)
    firebase.put('/', "date", my_date)
    firebase.put('/', "location", "Universitas Brawijaya")
    firebase.put('/', "time", current_time)

def sendNormal():
    firebase.put('/', "status", 0)
    firebase.put('/', "date", my_date)
    firebase.put('/', "location", "Universitas Brawijaya")
    firebase.put('/', "time", current_time)

def getData():
    currentStatus = firebase.get('/status', None)
    print(currentStatus)
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for obj in res[0][0]:
                ...
                if obj[2] > args.prob_threshold:
                    ...
                    # th0 = threading.Thread(target=getData)
                    # th0.start()
                    if class_id == 2 or class_id == 3:
                        # cv2.imwrite('image.jpg', frame)
                        th1 = threading.Thread(target=sendFirebase)
                        th1.start()
                        
                        print("Crime Detected !!")
                    else:
                        th2 = threading.Thread(target=sendNormal)
                        th2.start()
